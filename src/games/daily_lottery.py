...
class DailyLottery:
    ...
    def _pay_lottery(self, number):
        ...
        for ticket in tickets:  # current EV: .795
            ...
            if ticket.number == number:  # pay 1st prize
                ...
                user.total_prize += TICKET_PRICE * 300
                ...
                user.total_prize += TICKET_PRICE * 100
                ...
                user.total_prize += TICKET_PRICE * 25
                ...
                user.total_prize += TICKET_PRICE * 5
    ...
    def buy_lottery(self, update, context):
        if update.message.chat.id == MEOW_GROUP_ID:
            ...
            current_user.total_ticket += len(numbers)
            self.session.add(current_user)
            for number in numbers:
                new_ticket = BuyLog(current_user.account_id, number)
    ...
    def print_lottery_stat(self, update, context):
        if update.message.chat.id == MEOW_GROUP_ID:
            ...
            for user in users:
                msg += f"{user.name}: {user.total_ticket:}장 | {user.total_prize:,} Ᵽ | {user.total_prize/user.total_ticket:.2f} Ᵽ/장\n"
            context.bot.send_message(chat_id=update.effective_chat.id,
                                     text=msg,
                                     reply_to_message_id=update.message.message_id,
                                     parse_mode='html')
